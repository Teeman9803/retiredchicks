...
if ($layout_type != 'metro' && $pagination_type == 'show_all') {
    if($total_item <= 0) {
        $item_per_page = -1;
    } else {
...
if (!$disable_source && isset($authors) && is_array($authors) && count($authors) > 0) {
...
<div class="grid-plus-container grid-<?php echo esc_attr($section_id); ?> <?php echo esc_attr($post_type); ?> <?php if('true' == $cate_multi_line): ?> grid-cate-multi-line<?php endif; ?>"
    ...
    >
        ...
        <div class="grid-items">
            <?php
            $index = $diff_y = $current_y = $prev_y = $url_index = 0;
            $crop_size = 600;
            ...
            if($posts->have_posts() && $item_per_page > 0):
